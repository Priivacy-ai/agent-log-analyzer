...
The public scan analyzes target-sized recent sessions per auto-discovered supported source selected by the local CLI. The current auto-discovered sources are Claude Code, Claude Desktop, Codex, GitHub Copilot, OpenCode, Claude Desktop MCP, Cursor, Kiro, and Google Antigravity. Selection uses bounded local metadata: file/session size and recency, targeting roughly 5 MB total per source while avoiding very large multi-file reads. The server receives only the generated sanitized report JSON after the user has had a chance to inspect it.

Cursor, Kiro, and Google Antigravity may also keep conversation state in VS Code-style SQLite stores. Codex may keep diagnostic rows in `logs*.sqlite`. GitHub Copilot may keep Copilot CLI session history under `~/.copilot` or `COPILOT_HOME`, and Copilot Chat session JSON under VS Code workspace storage. Those stores are raw-sensitive: they can contain prompts, drafts, code, tool inputs, tool outputs, workspace paths, provider metadata, extension state, diagnostics, and error text. The CLI includes readable supported stores in automatic discovery, opens SQLite stores in read-only mode, and extracts only known bounded key prefixes, diagnostic columns, or session files. It must not modify source stores or write database snapshots. Raw database keys, raw paths, unknown state rows, OAuth/session material, protobuf blobs, and transcript lines are not emitted in reports.
