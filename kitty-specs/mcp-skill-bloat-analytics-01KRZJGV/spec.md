...
| C-005 | proposed | Implementation MUST happen on a single mission-scoped branch (not directly on `main`). The brief proposed `codex/mcp-skill-utilization` (timestamp-suffixed if it already exists), and that name SHOULD be used when the Spec Kitty `/spec-kitty.implement` workflow is not used. When the Spec Kitty workflow IS used, its canonical `kitty/mission-<mission_slug>` plus per-lane `-lane-<X>` suffix branches satisfy this constraint by construction. Either naming scheme is acceptable; what matters is that all implementation work lives on a single mission-scoped branch separable from `main`. |
