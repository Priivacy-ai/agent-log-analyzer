...
| FR-005 | Proposed | The registry MUST seed entries for all 20 named SDD tools (Spec Kitty, GitHub Spec Kit, OpenSpec, Kiro, BMAD-METHOD, GSD, Spec Workflow MCP, SDD Pilot, Spec-Driven Develop, spec2ship, ChatDev, PAUL, fspec, whenwords, Intent, Cognition/Devin, Microsoft Agent Framework, Tessl, Agentic Code, CodeSpeak). A tool's "seed entry" is satisfied by EITHER a verified production detector in `internal/analyzer/signatures/sdd_detectors*.json` OR a research stub in `docs/research/sdd-fingerprints/<id>.md` with status `research_needed` and a documented A-04 escape-hatch reason. Per FR-013, production detectors MUST NOT ship for `research_needed` tools — the research stub is the seed for those entries. |
