...
pushed ECR manifest, resolves the ECR digest, registers fresh ECS task
definition revisions pinned to `repository@sha256:...`, and only then updates
the ECS services. This prevents Apple Silicon `linux/arm64` images from reaching
Fargate and avoids mutable `latest` races where tasks can start on an older
digest than the one intended for the deploy.
